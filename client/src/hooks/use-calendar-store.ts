...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  generateAutoEvents: async () => {
    ...
    const existingAutoIds = new Set(existingAutoEvents.map((e) => e.id));
    ...
    if (eventsToAdd.length > 0 || eventsToUpdate.length > 0) {
      set((state) => {
        ...
        for (const updated of eventsToUpdate) {
          const idx = updatedEvents.findIndex((e) => e.id === updated.id);
          if (idx >= 0) {
            updatedEvents[idx] = updated;
          }
        }
        
        return { events: [...updatedEvents, ...eventsToAdd] };
      });
    }
  },
